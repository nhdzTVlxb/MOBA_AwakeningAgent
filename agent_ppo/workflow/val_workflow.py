...
def run_validation(agent, env, usr_conf, logger, episode_cnt=0):
    ...
    while not done:
        ...
        # 从 remain_info 获取 shaped_reward
        shaped_reward = remain_info.get("shaped_reward", 0.0)
        
        # 更新指标
        reward = remain_info.get("reward", [0.0])[0] if isinstance(remain_info.get("reward"), list) else remain_info.get("reward", 0.0)
        total_reward += reward
        ...
        metrics.update(
            ...
            is_flash=is_flash,
            is_flash_legal=True,  # 验证时假设都合法
            flash_ready=True       # 简化处理
        )
