...
###########################################################################
"""
DMSD: Dual-domain Memory and Situational Decision.

1. Local immediate domain:
   semantic_map encodes local walkability, visit heat, treasure, buff,
   monster risk, openness, corridor, and dead-end risk.

2. Semi-global memory domain:
   visit_heat, treasure_memory, buff_memory, position_history, and
   recent_positions accumulate information across the episode.

3. Situational decision signals:
   danger_level, pinch_risk, survival_pressure, greed_window,
   stagnation_ratio, oscillation_ratio, and topology summaries transform
   raw observations into interpretable risk/reward quantities.
"""
"""
...
class TargetMemory:
    ...
    last_seen_step: int = -1
        # buff 刷新记忆：主要给 sub_type == 2 使用
    
    last_unavailable_step: int = -1
    estimated_ready_step: int = -1
    last_picked_step: int = -1
...
class Preprocessor:
    ...
    def feature_process(self, env_obs, last_action):
        ...
        buff_available = (
            1.0
            if buff_target is not None and buff_target.available
            else 0.0
        )
    ...
    def _compute_buff_safety(
        ...
        核心规则：
        1. buff 已可用：只要距离别太远，并且怪追上时间 > 到达时间 + 安全余量，就可以追。
        2. buff 未刷新：只有 ready_in 很短，并且怪追上时间 > ready_in + 到达时间 + 安全余量，才允许短等。
        3. 剩余刷新时间太久，不允许原地等。
        """
        if buff_target is None or not _is_valid_position(buff_target.pos):
            return {
                "can_chase": False,
                "can_wait": False,
                "catch_steps": 0.0,
        ...
        monster_speed = max(float(monster_speed), 1.0)
        catch_steps = float(min_monster_distance) / monster_speed

        ready_step = getattr(buff_target, "estimated_ready_step", -1)
        if buff_target.available:
            ready_in = 0.0
        elif ready_step >= 0:
            ready_in = max(0.0, float(ready_step - self.step_no))
        else:
            ready_in = 999.0

        # 估算到达步数；如果闪现已好且距离在 10~12 步内，认为 1 步可到
        if flash_cooldown <= 1e-6 and buff_target.distance <= Config.BUFF_FLASH_PICKUP_DISTANCE:
            travel_steps = 1.0
        else:
            travel_steps = buff_target.distance / float(max(1.0, Config.BUFF_ESTIMATED_HERO_SPEED))

        too_far = buff_target.distance > Config.BUFF_MAX_CHASE_DISTANCE
        wait_too_long = ready_in > Config.BUFF_WAIT_MAX_STEPS

        safe_enough = catch_steps > (
            ready_in + travel_steps + Config.BUFF_WAIT_SAFE_MARGIN_STEPS
        )

        can_chase = (
            buff_target.available
            and not too_far
            and safe_enough
        )

        can_wait = (
            not buff_target.available
            and not too_far
            and not wait_too_long
            and safe_enough
        )

        if can_chase:
            reason = "can_chase_ready_buff"
        elif can_wait:
            reason = "can_wait_short_refresh"
        elif wait_too_long:
            reason = "refresh_too_long"
        elif not safe_enough:
            reason = "monster_catches_before_buff"
        elif too_far:
            reason = "buff_too_far"
        else:
            reason = "not_worth"
    ...
    def _sync_collectible_memory(self, organs, env_info, hero_pos):
        # 先清空本帧可见标记，同时根据 buff 刷新时间恢复可用性
        for memory_bank in (self.treasure_memory, self.buff_memory):
            for target in memory_bank.values():
                target.visible_this_step = False

                if hasattr(target, "just_refreshed"):
                    target.just_refreshed = False

                # buff：如果之前被吃掉/冷却中，并且已到估算刷新时间，则重新视为可用
                if (
                    target.sub_type == 2
                    and target.found
                    and not target.available
                    and getattr(target, "estimated_ready_step", -1) >= 0
                    and self.step_no >= target.estimated_ready_step
                ):
                    target.available = True
                    target.just_refreshed = True
        ...
        for organ in organs or []:
            ...
            if pos is not None and status != -1:
                ...
                if sub_type == 2 and not target.first_seen_rewarded:
                    target.first_seen_rewarded = True
                    newly_seen_buffs += 1

            elif not target.found and target.direction > 0:
                target.pos = _estimate_position_from_relative(
                    hero_pos,
                    target.direction,
                    target.distance_bucket,
                )
            ...
            if sub_type == 1:
                ...
                # buff：
                # status == 0 认为当前不可用/冷却中；
                # status != 0 认为当前可用。
                if status == 0:
                    target.available = False

                    if getattr(target, "last_unavailable_step", -1) < 0:
                        target.last_unavailable_step = self.step_no
                        target.estimated_ready_step = self.step_no + Config.BUFF_REFRESH_STEPS

                    # 如果之前估算已经过期，但环境仍显示不可用，则重新从当前步估算
                    elif self.step_no > target.estimated_ready_step + 5:
                        target.last_unavailable_step = self.step_no
                        target.estimated_ready_step = self.step_no + Config.BUFF_REFRESH_STEPS

                else:
                    was_unavailable = not target.available
                    target.available = True

                    if (
                        was_unavailable
                        or (
                            getattr(target, "estimated_ready_step", -1) >= 0
                            and self.step_no >= target.estimated_ready_step
                        )
                    ):
                        target.just_refreshed = True
    ...
    def _pick_best_buff_target(self):
        """
        DMSD buff target selection.

        和宝箱不同，buff 有刷新机制：
        1. 已可用的 buff 优先；
        2. 如果 buff 暂时不可用，但已经记住位置，并且有 estimated_ready_step，
           也允许作为候选，让 _compute_buff_safety 判断是否值得短等。
        """
        candidates = []
        for target in self.buff_memory.values():
            if not _is_valid_position(target.pos):
                continue
            ...
            ready_step = getattr(target, "estimated_ready_step", -1)
            if target.found and ready_step >= 0:
                ready_in = max(0.0, float(ready_step - self.step_no))

                # 只把“快刷新”的 buff 当目标；太久才刷新的不要占用 buff_target
                if ready_in <= Config.BUFF_WAIT_MAX_STEPS:
                    candidates.append(target)

        if not candidates:
            return None

        def _buff_key(target):
            if target.available:
                ready_in = 0.0
                available_rank = 0
            else:
                ready_in = max(
                    0.0,
                    float(getattr(target, "estimated_ready_step", -1) - self.step_no),
                )
                available_rank = 1
        ...
        return min(candidates, key=_buff_key)
...
#
    ...
    def _build_semantic_map(self, map_info, hero_pos, monsters):
        ...
        # 2: 宝箱通道，只放宝箱，正值
        semantic_map[2] = self._build_treasure_channel(hero_pos)

        # 3: buff 通道，只放 buff，正值
        semantic_map[3] = self._build_buff_channel(hero_pos)

        # 4: 怪物风险
        semantic_map[4] = self._build_risk_channel(hero_pos, monsters)

        # 5/6/7: 地形结构
        semantic_map[5] = openness_channel
        semantic_map[6] = corridor_channel
        semantic_map[7] = dead_end_channel

        topology_summary = self._summarize_topology(
            openness_channel,
            corridor_channel,
            dead_end_channel,
        )
    ...
    def _build_treasure_channel(self, hero_pos):
        """
        宝箱专用通道：
        1.0 = 已确认位置/当前可见过的宝箱
        0.5 = 只根据方向距离估计的位置
        ...
        for target in self.treasure_memory.values():
            if target.available and _is_valid_position(target.pos):
                ...
                value = 1.0 if target.found else 0.5
                self._mark_channel(channel, row, col, value)

        return channel


    def _build_buff_channel(self, hero_pos):
        """
        buff 专用通道：
        1.0 = 已确认位置/当前可见过的 buff
        0.5 = 只根据方向距离估计的位置

        注意：这里全部是正值，不再用 -1.0 / -0.5。
        """
        channel = np.zeros((Config.LOCAL_MAP_SIZE, Config.LOCAL_MAP_SIZE), dtype=np.float32)
...
######################
    def _build_reward(
        ...
        if (
            treasure_target is None
            and buff_target is not None
            and buff_target.available
            and self.last_target_buff_distance is not None
        ):
        ...
        buff_safety = self._compute_buff_safety(
            buff_target=buff_target,
            min_monster_distance=min_monster_distance,
            monster_speed=monster_speed,
        ...
        buff_flash_pickup_bonus = 0.0
        buff_refresh_pickup_bonus = 0.0
        buff_wait_penalty = 0.0

        buff_gained = buff_count > self.last_buff_count

        if buff_target is not None:
            buff_can_chase = bool(buff_safety["can_chase"])
            buff_can_wait = bool(buff_safety["can_wait"])

            # 只有“已刷新可追”或“很快刷新且安全”时，才奖励靠近 buff。
            # 这样不会鼓励原地等 200 步。
            if (buff_can_chase or buff_can_wait) and self.last_target_buff_distance is not None:
                buff_approach_gain = self.last_target_buff_distance - buff_target.distance
                if buff_approach_gain > 0:
                    distance_scale = min(buff_approach_gain / TARGET_DIST_SCALE, 1.0)

                    # 越近、越刚刷新、越后期，buff 价值越高，但不爆炸。
                    close_ratio = 1.0 - float(
                        np.clip(
                            buff_target.distance / float(max(1.0, Config.BUFF_MAX_CHASE_DISTANCE)),
                            0.0,
                            1.0,
                        )
                    )
                    ...
                    if getattr(buff_target, "just_refreshed", False):
                        buff_approach_reward *= 1.25

            # buff 未好，但智能体在附近磨蹭/撞墙/小循环，并且怪会先追上：惩罚
            if (
                not buff_target.available
                and not buff_can_wait
                and buff_safety["ready_in"] > 0
                and buff_safety["ready_in"] <= Config.BUFF_REFRESH_STEPS
            ):
                waiting_like = (
                    movement_state["stagnation_ratio"] > 0.35
                    or movement_state["oscillation_ratio"] > 0.25
                    or movement_state["hit_wall"]
                    or movement_state.get("small_loop", False)
                )

                if waiting_like:
                    buff_wait_penalty = -Config.BUFF_WAIT_PENALTY

                    if buff_safety["reason"] == "monster_catches_before_buff":
                        buff_wait_penalty -= Config.BUFF_DANGEROUS_WAIT_PENALTY

        if buff_gained:
            self.last_buff_effect_step = self.step_no

            buff_was_refreshed = (
                buff_target is not None
                and getattr(buff_target, "just_refreshed", False)
            )

            # 拿到后，把这个 buff 记为进入 200 步刷新 CD
            if buff_target is not None:
                buff_target.available = False
                buff_target.last_picked_step = self.step_no
                buff_target.last_unavailable_step = self.step_no
                buff_target.estimated_ready_step = self.step_no + Config.BUFF_REFRESH_STEPS
                buff_target.just_refreshed = False

            if danger_level >= Config.BUFF_HIGH_PRESSURE_THRESHOLD:
                buff_pickup_priority_bonus += Config.BUFF_HIGH_PRESSURE_PICKUP_BONUS

            if buff_was_refreshed:
                buff_refresh_pickup_bonus += Config.BUFF_REFRESH_PICKUP_BONUS

            # 关键：闪现拿 buff 给专项奖励
            if flash_used:
                buff_flash_pickup_bonus += Config.BUFF_FLASH_PICKUP_BONUS

                # 只要这次闪现确实拿到了 buff，就不要再按普通非穿墙闪现惩罚
                non_wall_flash_base_penalty *= Config.BUFF_RESOURCE_FLASH_PENALTY_SCALE
                flash_waste_penalty = 0.0
                safe_flash_penalty = 0.0
                blind_flash_penalty = 0.0
                early_flash_penalty = 0.0
            ...
            # 所有“拾取情景 bonus”封顶，避免 buff 总奖励远超宝箱
            buff_pickup_priority_bonus = min(
                buff_pickup_priority_bonus,
                Config.BUFF_PICKUP_BONUS_CAP,
            )
        ...
        reward_terms = {
            ...
            "buff_flash_pickup_bonus": buff_flash_pickup_bonus,
            "buff_refresh_pickup_bonus": buff_refresh_pickup_bonus,
            "buff_wait_penalty": buff_wait_penalty,
            "buff_can_chase": 1.0 if buff_safety["can_chase"] else 0.0,
            "buff_can_wait": 1.0 if buff_safety["can_wait"] else 0.0,
            "buff_ready_in": buff_safety["ready_in"],
            "buff_catch_steps": buff_safety["catch_steps"],
        }
        ...
        reward = float(
            sum(
                v for k, v in reward_terms.items()
                if k not in (
                    "stall_window_distance",
                    "buff_can_chase",
                    "buff_can_wait",
                    "buff_ready_in",
                    "buff_catch_steps",
