...
class Preprocessor:
    ...
    def feature_process(self, env_obs, last_action):
        ...
        self.last_local_openness = (
            topology_summary["local_openness"] if topology_summary is not None else 0.0
        )
...
#
    ...
    def _build_semantic_map(self, map_info, hero_pos, monsters):
        ...
        bfs_channel = self._build_bfs_channel(walkable_channel)
        ...
        semantic_map[7] = bfs_channel

        topology_summary = self._summarize_topology(
            openness_channel, corridor_channel, dead_end_channel
        )
    ...
    def _build_bfs_channel(self, walkable_channel):
        """
        从局部地图中心（英雄所在格）出发，做 8 邻域 BFS。
        输出一个 [LOCAL_MAP_SIZE, LOCAL_MAP_SIZE] 的通道：
        - 障碍/不可达 = 0.0
        - 可达格子 = 按 BFS 距离归一化，越近越大，越远越小
        """
        size = Config.LOCAL_MAP_SIZE
        center = LOCAL_HALF

        bfs_channel = np.zeros((size, size), dtype=np.float32)

        # 如果英雄脚下不可走，直接返回全 0
        if walkable_channel[center, center] <= 0.0:
            return bfs_channel

        dist = -np.ones((size, size), dtype=np.int32)
        q = deque()

        dist[center, center] = 0
        q.append((center, center))

        directions = list(ACTION_TO_ROW_COL_DELTA.values())
        ...
        reachable_mask = dist >= 0
        if not np.any(reachable_mask):
            return bfs_channel

        max_dist = max(1, int(dist[reachable_mask].max()))

        bfs_channel[reachable_mask] = 1.0 - (
            dist[reachable_mask].astype(np.float32) / float(max_dist)
        )
        bfs_channel[center, center] = 1.0
