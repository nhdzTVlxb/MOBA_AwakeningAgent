...
class MapManager:
    ...
    def update_organs(self, treasures, buff, hero_pos):
        """更新宝箱和buff在地图上的位置"""
        self.treasures.fill(0.0)
        self.buffs.fill(0.0)
        
        for t in treasures:
            if t.available and t.pos[0] != -1:
                x, z = int(round(t.pos[0])), int(round(t.pos[1]))
                if 0 <= x < 128 and 0 <= z < 128:
                    self.treasures[x, z] = 1.0
        
        if buff.available and buff.pos[0] != -1:
            x, z = int(round(buff.pos[0])), int(round(buff.pos[1]))
            if 0 <= x < 128 and 0 <= z < 128:
                self.buffs[x, z] = 1.0
    ...
    def is_walkable(self, x, z):
        ...
        if 0 <= ix < 128 and 0 <= iz < 128:
            return self.obstacles[ix, iz] <= 0
        return False
    
    def check_line_crosses_obstacle(self, start_pos, end_pos, num_samples=12):
        """检查从起点到终点的线段是否穿过障碍物"""
        x1, z1 = start_pos
        x2, z2 = end_pos
        
        start_walkable = self.is_walkable(x1, z1)
        end_walkable = self.is_walkable(x2, z2)
...
class Preprocessor:
    ...
    def reset(self):
        ...
        self.last_hero_pos_for_mask = None
        self.last_hero_pos_for_reward = None
        ...
        self.monster_memory = [
            {'last_seen_pos': None, 'last_seen_step': 0, 'last_seen_dist': 0.0},
            {'last_seen_pos': None, 'last_seen_step': 0, 'last_seen_dist': 0.0}
        ]
        
        self._debug_printed = False
        self._debug_monster_id = False
        
        # 动态配置（由课程学习设置）
        self.current_speed_up_step = Config.MONSTER_SPEED_UP_STEP
        self.current_monster2_spawn_step = Config.MONSTER2_SPAWN_STEP

        # ========== 闪现教学相关状态 ==========
        self.wall_hit_streak = 0
        self._last_step_hit_wall = False

        # 上一帧的闪现先验信息
        self.prev_dead_end_risk = 0.0
        self.prev_monster_closing = 0.0
        self.prev_flash_high_value = 0.0
        self.prev_is_speedup = False
    ...
    def set_episode_config(self, speed_up_step=None, monster2_spawn_step=None):
        """设置当前 episode 的动态配置（由课程学习调用）"""
        if speed_up_step is not None:
            self.current_speed_up_step = speed_up_step
        if monster2_spawn_step is not None:
            self.current_monster2_spawn_step = monster2_spawn_step

    def _update_hit_state(self, hero_pos):
        """根据上一动作结果更新连续撞墙次数"""
        self._last_step_hit_wall = False

        if self.last_hero_pos_for_mask is not None and self.last_action != -1 and self.last_action < 8:
            dx = self.last_hero_pos_for_mask[0] - hero_pos[0]
            dz = self.last_hero_pos_for_mask[1] - hero_pos[1]

            if abs(dx) < 0.1 and abs(dz) < 0.1:
                self._last_step_hit_wall = True
                self.wall_hit_streak = min(self.wall_hit_streak + 1, 5)
            else:
                self.wall_hit_streak = 0
        elif self.last_action >= 8:
            self.wall_hit_streak = 0

    def _calc_flash_high_value_window(
        self,
        wall_hit_streak,
        dead_end_risk,
        monster_closing,
        ...
        if wall_hit_streak < Config.FLASH_HIT_STREAK_THRESHOLD:
            return 0.0

        danger_dist = Config.FLASH_POST_DANGER_DIST if is_speedup else Config.FLASH_PRE_DANGER_DIST
        if min_dist >= danger_dist:
            return 0.0

        if dead_end_risk < 0.45 and monster_closing < 0.2:
            return 0.0

        return 1.0

    def _calc_treasure_pickup_reward(self, treasure_get):
        """前3个宝箱分段奖励：3.5 / 2.5 / 1.5，后续每个1.0"""
        if treasure_get <= 0:
            return 0.0

        reward = 0.0
        start_count = self.last_treasure_count
        end_count = self.last_treasure_count + treasure_get

        for idx in range(start_count + 1, end_count + 1):
            if idx <= len(Config.TREASURE_REWARD_SCHEDULE):
                reward += Config.TREASURE_REWARD_SCHEDULE[idx - 1]
            else:
                reward += Config.TREASURE_REWARD_FALLBACK
    ...
    def feature_process(self, env_obs, last_action):
        ...
        # 先更新"最近撞墙状态"
        self._update_hit_state(hero_pos)
        ...
        self.map_manager.update_organs(self.treasures, self.buff, hero_pos)
        ...
        current_dead_end_risk = 1.0 - self._calc_openness_score(hero_pos)

        current_monster_closing = 0.0
        if self.last_min_dist is not None and current_min_dist < 1000:
            current_monster_closing = max(
                0.0,
                min((self.last_min_dist - current_min_dist) / 5.0, 1.0)
            )

        flash_ready = float(talent.get('status', 0) == 1)
        is_speedup = float(self.step_no >= self.current_speed_up_step)

        current_flash_high_value = self._calc_flash_high_value_window(
            wall_hit_streak=self.wall_hit_streak,
            dead_end_risk=current_dead_end_risk,
            monster_closing=current_monster_closing,
            min_dist=current_min_dist,
            flash_ready=flash_ready > 0.5,
            is_speedup=is_speedup > 0.5
        )

        if self.step_no == 50 and not self._debug_printed:
            self._debug_printed = True
            monsters = frame_state.get('monsters', [])
            for i, m in enumerate(monsters):
                m_pos = m.get('pos', {})
                m_x, m_z = m_pos.get('x', 0), m_pos.get('z', 0)
                dx, dz = m_x - hero_pos[0], m_z - hero_pos[1]
                dist = math.sqrt(dx*dx + dz*dz)
                in_view = dist <= 10.0
                msg = f"[MONSTER_OBS_DEBUG] step={self.step_no} monster_{i} pos=({m_x:.1f},{m_z:.1f}) dist={dist:.1f} in_view={in_view}"
                if self.logger:
                    self.logger.info(msg)
                else:
                    print(msg)

        if self.step_no == 50 and not self._debug_monster_id:
            self._debug_monster_id = True
            monsters = frame_state.get('monsters', [])
            for i, m in enumerate(monsters):
                ids = {k: m.get(k) for k in ['config_id', 'runtime_id', 'uid', 'actor_id', 'id'] if k in m}
                if ids:
                    msg = f"[MONSTER_ID_DEBUG] step={self.step_no} monster_{i} ids={ids}"
                    if self.logger:
                        self.logger.info(msg)
                    else:
                        print(msg)
        ...
        hero_feat = np.array([
            ...
            # ========== 新增4维：闪现先验特征 ==========
            min(self.wall_hit_streak / 3.0, 1.0),   # 最近连续撞墙次数（归一化）
            current_dead_end_risk,                  # 当前死角风险
            current_monster_closing,                # 最近怪物是否在逼近
            current_flash_high_value,               # 是否处于高价值闪现窗口
        ], dtype=np.float32)

        # ========== 2. 怪物特征 - 按每只怪 10 维拼接（原始5 + 记忆5） ==========
        monster_feats = []
        ...
        for i in range(2):
            ...
            if i < len(monsters):
                ...
                if in_view:
                    self.monster_memory[i]['last_seen_pos'] = [m_x, m_z]
                    self.monster_memory[i]['last_seen_step'] = self.step_no
                    self.monster_memory[i]['last_seen_dist'] = distance
                ...
                monster_feats.extend([
                    ...
                    norm(distance, 0, 180),
                    float(self.step_no >= self.current_speed_up_step)
                ])
            else:
            ...
            if mem['last_seen_pos'] is not None:
                mem_dx = mem['last_seen_pos'][0] - hero_pos[0]
                mem_dz = mem['last_seen_pos'][1] - hero_pos[1]
                mem_dist = math.sqrt(mem_dx*mem_dx + mem_dz*mem_dz)
                steps_since = self.step_no - mem['last_seen_step']
                confidence = max(0.0, 1.0 - steps_since / 50.0)
                ...
                if i < len(monsters):
                    ...
                    dx, dz = m_x - hero_pos[0], m_z - hero_pos[1]
                    dist = math.sqrt(dx*dx + dz*dz)
                    visible_now = dist <= 10.0
                
                monster_feats.extend([
                    float(visible_now),
                    norm(mem_dx / max(mem_dist, 1e-4), -1, 1),
                    norm(mem_dz / max(mem_dist, 1e-4), -1, 1),
                    norm(mem_dist, 0, 180),
                    confidence
                ])
            else:
        ...
        # 5. 主目标宝箱特征 (4维)
        primary_treasure_feat = np.array([0.0, 0.0, 1.0, 0.0], dtype=np.float32)
        for feat in treasure_feats:
            if feat[0] > 0:
                primary_treasure_feat = np.array([
                    feat[1],
                    feat[2],
                    feat[3],
                    1.0,
        ...
        is_speedup_flag = float(self.step_no >= self.current_speed_up_step)
        progress_feat = np.array([step_norm, is_speedup_flag], dtype=np.float32)
        ...
        feature = np.concatenate([
            ...
            map_feat,
            treasure_feat,
            primary_treasure_feat,
            buff_feat,
            np.array(legal_action, dtype=np.float32),
            progress_feat
        ])

        total_reward, shaped_reward, distance_reward, min_dist, nearest_treasure_dist = self._get_reward(
        ...
        # ========== 为下一步动作决策保存当前闪现先验 ==========
        self.prev_dead_end_risk = current_dead_end_risk
        self.prev_monster_closing = current_monster_closing
        self.prev_flash_high_value = current_flash_high_value
        self.prev_is_speedup = bool(is_speedup)
        ...
        return feature, legal_action, [total_reward], shaped_reward, visible_treasure_ratio, distance_reward, remain_info

    def _get_action_mask(self, hero_info, hero_pos):
        mask = [1] * Config.ACTION_NUM

        if self.last_action != -1 and self.last_action < 8:
            if self._last_step_hit_wall:
    ...
    def _calc_second_monster_penalty(self, hero_pos, monsters):
        ...
        is_speedup = self.step_no >= self.current_speed_up_step
    ...
    def _calc_pre_speedup_bonus(self):
        steps_to_speedup = self.current_speed_up_step - self.step_no
    ...
    def _calc_flash_over_wall_reward(self, hero_pos):
        if self.last_action < 8:
            return 0.0
        
        if self.last_hero_pos_for_reward is None:
            return 0.0
        
        start_pos = self.last_hero_pos_for_reward
        end_pos = hero_pos
        
        dx = end_pos[0] - start_pos[0]
        dz = end_pos[1] - start_pos[1]
        flash_dist = math.sqrt(dx*dx + dz*dz)
        
        if flash_dist < 2.0:
            return 0.0
        
        if self.map_manager.check_line_crosses_obstacle(start_pos, end_pos):
            return Config.REW_FLASH_OVER_WALL
        
        return 0.0

    def _calc_flash_reward_detailed(self, hero_pos, min_dist, treasure_get):
        reward = 0.0
        is_flash = self.last_action >= 8
        ...
        openness_gain = current_openness > last_openness + 0.3
        openness_drop = current_openness < last_openness - 0.3

        over_wall_reward = self._calc_flash_over_wall_reward(hero_pos)
        over_wall = over_wall_reward > 0.0

        dist_gain = False
        if self.last_min_dist is not None:
            dist_gain = min_dist > self.last_min_dist + 5

        # ========== 情况1：模型自己选择了闪现 ==========
        if is_flash:
            reward += over_wall_reward
            ...
            if treasure_get > 0:
                reward += Config.REW_TREASURE * 0.05

            # ========== 新增：高价值闪现窗口中的定向奖励 ==========
            if self.prev_flash_high_value > 0.5:
                if self.prev_is_speedup:
                    # 高压版（加速后）
                    if over_wall:
                        reward += Config.REW_FLASH_POST_WALL
                    if dist_gain:
                        reward += Config.REW_FLASH_POST_ESCAPE
                    if over_wall and dist_gain:
                        reward += Config.REW_FLASH_POST_COMBO
                else:
                    # 低压版（加速前）
                    if over_wall:
                        reward += Config.REW_FLASH_PRE_WALL
                    if dist_gain:
                        reward += Config.REW_FLASH_PRE_ESCAPE
                    if openness_gain:
                        reward += Config.REW_FLASH_PRE_OPEN
        ...
        # ========== 情况2：明明是高价值闪现窗口，却没闪 ==========
            ...
            if self.prev_flash_high_value > 0.5:
                if self.prev_is_speedup:
                    reward -= Config.PENALTY_FLASH_HOLD_POST
                    if min_dist < 6:
                        reward -= 0.1
                else:
                    reward -= Config.PENALTY_FLASH_HOLD_PRE
    ...
    def _get_reward(self, hero_pos, monsters, treasure_get, buff_get):
        ...
        if treasure_get > 0:
            treasure_pickup = self._calc_treasure_pickup_reward(treasure_get)
        ...
        return total_reward, shaped_reward, distance_reward, min_dist, nearest_treasure_dist