...
class EpisodeRunner:
    ...
    def run_episodes(self):
        while True:
            ...
            # ========== 2. 把课程参数写入环境配置（字段名与 TOML 对齐） ==========
            env_conf = self.usr_conf.get('env_conf', {})
            env_conf['treasure_count'] = stage_config['treasure_num']
            env_conf['buff_count'] = stage_config['buff_num']
            env_conf['monster_interval'] = stage_config['monster2_spawn_step']
            env_conf['monster_speedup'] = stage_config['monster_speed_up_step']
            ...
            # 注意：这里保留 map_id 覆盖逻辑
            # 如果你的环境实际不认 map_id，而只认 map/map_random，
            # 需要再单独适配环境接口
            ...
            # ========== 5. 先设置 preprocessor 配置，再生成首帧观测 ==========
            observation = env_obs.get("observation", {})
            frame_state = observation.get("frame_state", {})
            _, hero_pos = get_hero_info_and_pos(frame_state)

            actual_speedup_step = env_conf['monster_speedup']
            actual_monster2_step = env_conf['monster_interval']

            self.agent.preprocessor.set_start_pos(hero_pos)
            self.agent.preprocessor.set_episode_config(
                speed_up_step=actual_speedup_step,
                monster2_spawn_step=actual_monster2_step
            )

            obs_data, remain_info = self.agent.observation_process(env_obs)

            metrics = EpisodeMetrics(actual_speedup_step)
            ...
            while not done:
                ...
                min_dist = _remain_info.get("min_monster_dist", 1000.0)
                ...
                if flash_ready:
                    flash_legal_steps += 1
                    if min_dist < 20:
                        flash_legal_danger_steps += 1

                if act_data.prob and len(act_data.prob) >= 16:
                    flash_prob_sum = sum(act_data.prob[8:16])
                    flash_action_probs.append(flash_prob_sum)
                ...
                if terminated:
                    ...
                    metrics.set_final_danger(min_dist)

                    nearest_treasure_dist = _remain_info.get("nearest_treasure_dist", 1000.0)
                    metrics.set_final_treasure_dist(nearest_treasure_dist)
                    ...
                    avg_flash_prob = sum(flash_action_probs) / len(flash_action_probs) if flash_action_probs else 0.0
                    ...
                    self.logger.info(
                        f"[FLASH_DIAG] Episode:{self.episode_cnt} | "
                        f"LegalSteps:{flash_legal_steps} | "
                        f"LegalDangerSteps:{flash_legal_danger_steps} | "
                        f"AvgFlashProb:{avg_flash_prob:.4f} | "
                        f"FlashUsed:{summary['flash_count']}"
                    )
                    ...
                    if now - self.last_report_monitor_time >= 60 and self.monitor:
                        monitor_data = {
                            ...
                            "flash_legal_steps": flash_legal_steps,
                            "flash_legal_danger_steps": flash_legal_danger_steps,
                            "avg_flash_prob": avg_flash_prob,
                        }
